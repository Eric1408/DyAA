...
void RamMachine::runProgram(bool trace) {
  ...
  while (!halt && !error_) {
    ...
    if (actual->getOp() == "read") {               // READ
    ...
    else if (actual->getOp() == "exp") {          // EXP
      switch (actual->getType()) {
      case INDIRECT:
        memory_->write(pow(memory_->read(ACCUM), memory_->read(memory_->read(stoi(actual->getOperand())))),ACCUM);
        break;
      case DIRECT:
        memory_->write(pow(memory_->read(ACCUM), memory_->read(stoi(actual->getOperand()))),ACCUM);
        break;
      case IMMEDIATE:
        memory_->write(pow(memory_->read(ACCUM), stoi(actual->getOperand())),ACCUM);
        break;
      default:
        break;
      }
      ...
      switch (actual->getType()) {
        case INDIRECT:
          if (memory_->read(memory_->read(stoi(actual->getOperand()))) == 0) {
          ...
          break;
        case DIRECT:
          if (memory_->read(stoi(actual->getOperand())) == 0) {
            cerr << "No se puede dividir por 0" << endl;
            error_ = true;
            break;
          }
